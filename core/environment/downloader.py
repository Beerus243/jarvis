...
class ArtifactDownloader:
    ...
    def download(self, artifact: InstallationArtifact):
        ...
        if Path(artifact.name).name != artifact.name or artifact.name in {'', '.', '..'}:
            return DownloadResult(False,error='Nom d’archive invalide.')
        ...
        try:
            for _ in range(4):
                ...
                if getattr(response,'is_redirect',False) or getattr(response,'is_permanent_redirect',False) or response.status_code in (301,302,303,307,308):
                    ...
                    if not location or urlparse(location).scheme != 'https' or urlparse(location).hostname not in ('storage.googleapis.com','nodejs.org','adoptium.net','api.adoptium.net','github.com','release-assets.githubusercontent.com','dl.google.com'): return DownloadResult(False,error='Redirection vers une source non autorisée.')
                ...
                if artifact.checksum_algorithm not in {'sha1','sha256','sha512'}:
                    return DownloadResult(False,error='Algorithme de checksum non pris en charge.')
                fd,tmp=tempfile.mkstemp(prefix='.jarvis-',dir=self.cache_dir); import os; os.close(fd); total=0; digest=hashlib.new(artifact.checksum_algorithm)
                ...
                if artifact.source.checksum and digest.hexdigest().lower()!=artifact.source.checksum.lower(): Path(tmp).unlink(missing_ok=True); return DownloadResult(False,error='Checksum invalide.')
