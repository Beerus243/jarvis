...
class JdkInstaller(EnvironmentInstaller):
    ...
    def artifact_from_research(self,research):
        ...
        item=research.artifacts[0]; source=TrustedSource('Eclipse Adoptium',item.version,item.artifact,item.download_url,item.checksum,item.architecture)
        return InstallationArtifact(item.artifact,item.version,item.platform,item.architecture,source,item.artifact.split('.')[-1],Path.home()/'.local/share/jarvis/environments/jdk'/item.version,item.checksum)
