...
def test_android_sdk_components_are_discovered(tmp_path,monkeypatch):
    root=tmp_path/'sdk'; (root/'platform-tools').mkdir(parents=True); adb=root/'platform-tools/adb'; adb.write_text('x'); adb.chmod(0o755); (root/'build-tools/34').mkdir(parents=True); (root/'platforms/android-34').mkdir(parents=True); manager=root/'cmdline-tools/latest/bin/sdkmanager'; manager.parent.mkdir(parents=True); manager.write_text('x'); manager.chmod(0o755)
