...
def test_android_sdk_components_are_discovered(tmp_path,monkeypatch):
    root=tmp_path/'sdk'; (root/'platform-tools').mkdir(parents=True); (root/'platform-tools/adb').write_text('x'); (root/'build-tools/34').mkdir(parents=True); (root/'platforms/android-34').mkdir(parents=True); (root/'cmdline-tools/latest').mkdir(parents=True)
